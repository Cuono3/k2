...
public class ProductModel {
	...
	public synchronized Collection<ProductBean> doRetrieveAll(String where) throws SQLException {
	    ...
	    try {
	        connection = DriverManagerConnectionPool.getConnection();
	        preparedStatement = connection.prepareStatement(selectSQL);
	        preparedStatement.setString(1, where); // Imposta il parametro 'where'
	        ...
	        while (rs.next()) {
	            ...
	            rs2.close();  // Chiude il ResultSet dopo l'uso
	            preparedStatement2.close();  // Chiude il PreparedStatement dopo l'uso
	            DriverManagerConnectionPool.releaseConnection(connection2);  // Rilascia la connessione
	        }
	        rs.close();  // Chiude il ResultSet dopo l'uso
